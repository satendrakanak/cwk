...
export function ArticleCard({ article }: ArticleCardProps) {
  ...
  return (
    <article className="academy-card group relative block h-full overflow-hidden p-3 transition-all duration-300 hover:-translate-y-1 hover:border-primary/25 hover:shadow-[0_30px_90px_color-mix(in_oklab,var(--primary)_18%,transparent)]">
      ...
      <div className="relative h-52 w-full overflow-hidden rounded-[24px] bg-muted">
        ...
        {category ? (
          <Link
            ...
            className="absolute left-3 top-3 rounded-full border border-white/20 bg-white/90 px-3 py-1 text-xs font-semibold text-primary shadow-sm backdrop-blur-md transition hover:bg-primary hover:text-primary-foreground"
          >
            {category.name}
          </Link>
        ) : null}
